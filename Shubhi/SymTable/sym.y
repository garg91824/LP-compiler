...
%token <char*> ID
%token NL SE C INT FLOAT CHAR DOUBLE
%%
START:S1 NL {return;}
;
S1:S NL S1
...
DOUBLE L4 E S
|
L1:L1 C ID{strcpy(temp,$3); insert(0);}
|ID{sprintf(temp,"%d",$1); insert(0);}
;
L2:L2 C ID{sprintf(temp,"%d",$3); insert(1);}
|ID{sprintf(temp,"%d",$1);  insert(1);}
;
L3:L3 C ID{sprintf(temp,"%d",$3); insert(2);}
|ID{sprintf(temp,"%d",$1);  insert(2);}
;
L4:L4 C ID{sprintf(temp,"%d",$3); insert(3);}
|ID{sprintf(temp,"%d",$1);insert(3);}
;
E:SE
;
%%
